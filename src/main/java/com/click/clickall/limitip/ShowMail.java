...
/*
     ...
     */
     public static String getAllEmail(String username,String password) throws Exception{
    	String msg = "";
    	// 承载信件
    	// ArrayList<ShowEmailMsg> showEmailMsgs = new ArrayList<ShowEmailMsg>();
    	 
    	// 新浪接收
        String host = "pop.sina.com";

/*        String username = "lyj51666@sina.com";

        String password = "liaoyangjun";*/
...
/*
        for (int i = 0; i < message.length; i++) {
            ShowEmailMsg showEmailMsg = new ShowEmailMsg();
            re = new ShowMail((MimeMessage) message[i]);
            // 主题
            showEmailMsg.setTitle(re.getSubject());
            // 时间
            showEmailMsg.setSendTime(re.getSentDate());
            // 是否已读
            if (re.isNew() == true) {
				// 未读
            	showEmailMsg.setIfRead("未读");
			}else if (re.isNew() == false) {
				// 已读
				showEmailMsg.setIfRead("已读");
			}
            ...
            *//*System.out.println("邮件　" + i + "　发送人地址:　" + re.getFrom());*//*
            // 发件人
            showEmailMsg.setSendUser(re.getFrom().split("<")[1].split(">")[0]);
            // 收件人
            showEmailMsg.setReciveUser(re.getMailAddress("to").split("<")[1].split(">")[0]);
            ...
            // 信息
            showEmailMsg.setEmailInfo(re.getBodyText());
            
            *//*re.setAttachPath("e:\\");*//*

            showEmailMsgs.add(showEmailMsg);
        }*/

         re = new ShowMail((MimeMessage) message[message.length-1]);
         re.getMailContent((Part) message[message.length-1]);
         msg = re.getBodyText();
        return msg;
    }
